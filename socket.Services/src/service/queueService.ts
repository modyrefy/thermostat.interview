...
function InitRabbitmqConnection(){
    amqp.connect(process.env.RABBIT_MQ_CONNECTION as string, function (error:any, connection:any) {
        if (error) {
            console.error("[AMQP]", error.message);
            return setTimeout(InitRabbitmqConnection, 5000);
            //throw error0;
        }
